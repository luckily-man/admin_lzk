<template>
  <div class="ux-table-tpl lang">
                  ...
                  >
                    <template slot-scope="props">
                          <el-tooltip effect="dark" :content="`ID : ${props.row.uuid}`" placement="top-start" v-if="column.type==='uuid'"  popper-class="atooltip">
                           ...
                           <span v-if="column.type==='size-unit'">{{props.row[column.prop]}}GB</span>
    ...
    <!-- $attrs 接受父作用域中传递过来的属性  可以直接传递下去 -->
    <!-- $listeners  包含了父作用域中非props特性绑定的属性，也就是在使用组件的时候在标签中定义的事件-->
